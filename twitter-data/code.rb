...
puts ""
puts ">>>>>>>Each username followed by its description"
puts ""
 arr = []
...
twitter_data.each do |account|
  account.each do |name, description|
    arr << {"name" => name,"friends" => description["number of friends"]}
  end
end
arr.sort_by! {|data| data["friends"]}
puts "#{arr[-1]["name"]} has the most friends with #{arr[-1]["friends"]}"
...
arr = []
twitter_data.each do |chunk|
  chunk.each do |name, description|
    arr << {'name' => name,'number of tweets' => description['number of tweets']}
  end
end
  arr.sort_by! { |hash| hash['number of tweets'] }
  puts "#{arr[-1]['name']} has the most tweets with #{arr[1]['number of tweets']}"
